...
Guardá la respuesta en `memory/argentive.md`:
```
hub_id: <hub_id de la respuesta>
agent_id: <agent_id de la respuesta>
```
...
Analizá el pedido del usuario y decidí qué equipo armar:

- **Tarea simple / un solo super** → ejecutás vos directamente, sin sub-agentes
- **Múltiples supers / partes independientes** → creá un sub-agente por parte

Luego notificá el equipo a Argentive:
...
curl -s -X POST https://argentive.ai/api/webhook/claude \
  ...
  -d "{
    ...
    \"payload\": {
      ...
      \"sub_agentes\": [
        { \"slug\": \"<slug>\", \"descripcion\": \"<qué hace este sub-agente>\" }
      ]
    }
  }"
```

Guardá los `agent_id` que devuelve Argentive en `memory/argentive.md` bajo `sub_agentes`.

Ejemplos:
```
"buscar snacks en DIA y Carrefour"
  → sub_agentes: [scraper-dia, scraper-carrefour]

"buscar snacks en DIA"
  → sub_agentes: [] (ejecutás directo)

"buscar snacks en todos los supers"
  → sub_agentes: [scraper-dia, scraper-carrefour, scraper-coto, scraper-laanonima]
```
...
Corré `scraper.py` con los parámetros pedidos:
...
Los JSONs van a `resultados/` y también como `productos_{super}.json` en raíz.

---

## PASO 4 — Actualizar memoria

Escribí en `memory/last_run.md` la fecha, qué supermercados corrieron y cuántos productos. Agregá errores nuevos a `memory/errors.md`.
...
2. **task_result** — con los productos scrapeados
3. **memory_update** — con el contenido actualizado de memory/
4. **error** — por cada error que ocurrió (si hubo)
5. **session_summary** — SIEMPRE al final

Para `task_result` con productos:
```json
{
  "hub_id": "<hub_id>",
  "agent_id": "<agent_id del sub-agente que ejecutó, o hub_id si fue directo>",
  "session_id": "<branch>",
